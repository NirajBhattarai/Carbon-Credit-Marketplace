...
import { db, usertable, eq } from '../lib/db';
...
function generateDeviceId(companyIndex: number, apiKeyIndex: number, deviceIndex: number): string {
  ...
  try {
    ...
    for (let companyIndex = 0; companyIndex < COMPANIES.length; companyIndex++) {
      ...
      await db.insert(usertable).values({
        ...
        target: usertable.walletAddress,
        set: {
          walletAddress: company.walletAddress,
        }
      });

      // TODO: Uncomment when applications, apiKeys, and iotDevices tables are added to schema
      /*
      // Create application

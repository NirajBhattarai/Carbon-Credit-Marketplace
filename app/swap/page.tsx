...
export default function SwapPage(): JSX.Element {
  ...
  const TokenSelector = ({ isOpen, onClose, onSelect, currentToken }: {
    isOpen: boolean
    onClose: () => void
            ...
            {tokens.map((token) => (
              <button
                ...
                className={`w-full flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 ${
                  token.symbol === currentToken?.symbol ? 'bg-blue-50 border border-blue-200' : ''
                }`}
              >
                <div className="flex items-center space-x-3">
                  {token.icon.startsWith('http') ? (
                    ...
                    <div className="w-8 h-8 flex items-center justify-center text-2xl">{token.icon}</div>
                  )}
  ...
  const SettingsModal = () => {
    ...
    return (
      <Modal isOpen={settingsModal.isOpen} onClose={settingsModal.close} size="md">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex space-x-2">
                {[0.1, 0.5, 1.0].map((value) => (
                  <Button
                    key={value}
                    onClick={() => settingsForm.setValue('slippageTolerance', value)}
                    variant={settingsForm.values.slippageTolerance === value ? 'primary' : 'outline'}
                    size="sm"
                  >
                    {value}%
                  </Button>
              ...
              <div className="mt-2">
                <Input
                  ...
                  onChange={(e) => settingsForm.setValue('slippageTolerance', parseFloat(e.target.value) || 0)}
                  placeholder="Custom"
            ...
            <div>
              <Input
                ...
                onChange={(e) => settingsForm.setValue('transactionDeadline', parseInt(e.target.value) || 20)}
                placeholder="20"
            ...
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-gray-700">Auto-refresh quotes</label>
              <input
                type="checkbox"
                checked={settingsForm.values.autoRefresh}
                onChange={(e) => settingsForm.setValue('autoRefresh', e.target.checked)}
                className="rounded border-gray-300"
              />
            </div>
          </div>
          <div className="mt-6 flex space-x-3">
            <Button onClick={settingsModal.close} variant="outline" className="flex-1">
              Cancel
            </Button>
            <Button onClick={handleSaveSettings} variant="primary" className="flex-1">
              Save Settings
            </Button>
          </div>
        </ModalContent>
    ...
    <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Trade Carbon Credits</h1>
          <p className="text-lg text-gray-600">Swap tokens instantly with the best rates</p>
        </div>
        
        <div className="flex justify-center">
          <div className="w-full max-w-md">
            <Card variant="elevated" padding="lg" className="bg-white/80 backdrop-blur-sm border-0 shadow-2xl">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-2xl font-bold text-gray-900">Trade</CardTitle>
                    <p className="text-sm text-gray-500 mt-1">Exchange tokens seamlessly</p>
                  </div>
                  <Button
                  ...
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0l1.83 7.508a2.5 2.5 0 01-2.35 3.175H9.5a2.5 2.5 0 01-2.35-3.175l1.83-7.508z" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-2xl p-5 border border-emerald-100">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-medium text-emerald-700">From</span>
                      <span className="text-sm text-emerald-600 font-medium">Balance: {fromToken?.balance || '0'}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <Input
                        type="number"
                      ...
                      >
                        {fromToken?.icon.startsWith('http') ? (
                          <img src={fromToken.icon} alt={fromToken.symbol} className="w-7 h-7" />
                        ) : (
                          <div className="w-7 h-7 flex items-center justify-center text-xl">{fromToken?.icon}</div>
                        )}
                        <span className="font-bold text-gray-900">{fromToken?.symbol}</span>
                  ...
                  <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-2xl p-5 border border-teal-100">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-medium text-teal-700">To</span>
                      <span className="text-sm text-teal-600 font-medium">Balance: {toToken?.balance || '0'}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <Input
                        type="number"
                      ...
                      >
                        {toToken?.icon.startsWith('http') ? (
                          <img src={toToken.icon} alt={toToken.symbol} className="w-7 h-7" />
                        ) : (
                          <div className="w-7 h-7 flex items-center justify-center text-xl">{toToken?.icon}</div>
                        )}
                        <span className="font-bold text-gray-900">{toToken?.symbol}</span>
                    ...
                    <div className="bg-gradient-to-r from-gray-50 to-slate-50 rounded-2xl p-5 space-y-4 border border-gray-200">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-bold text-gray-900">
                          1 {fromToken.symbol} = {((toToken.priceUSD || 0) / (fromToken.priceUSD || 1)).toFixed(6)} {toToken.symbol}
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-gray-600">Price Impact</span>
                        <Badge variant={priceImpact > 5 ? 'error' : priceImpact > 1 ? 'warning' : 'success'}>
                          {priceImpact}%
                        </Badge>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-gray-600">Minimum Received</span>
                        <span className="text-sm font-bold text-gray-900">{minimumReceived} {toToken.symbol}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-gray-600">Network Fee</span>
                        <span className="text-sm font-bold text-gray-900">{networkFee}</span>
                      </div>
                    </div>
                  )}

...
import { mockNFTs, mockCollections, oceanConservationNFTs } from '@/lib/mockData'
import { CarbonCreditNFT, Collection } from '@/lib/types'

interface FilterOptions {
  sortBy: 'price-low' | 'price-high' | 'recent' | 'oldest' | 'rarity' | 'credits-high' | 'credits-low'
...
export default function NFTsPage() {
  ...
  const calculateTrendingScore = (collection: Collection): number => {
    ...
    const recentActivity = nfts.filter(nft => {
      const daysSinceCreated = (Date.now() - new Date(nft.createdAt).getTime()) / (1000 * 60 * 60 * 24)
      return daysSinceCreated <= 30
    }).length
    
    const avgPrice = nfts.reduce((sum, nft) => sum + nft.price, 0) / nfts.length
  ...
  const filteredNFTs = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(nft => 
        ...
        nft.carbonCredits.projectType.toLowerCase().includes(searchQuery.toLowerCase()) ||
        nft.carbonCredits.location.toLowerCase().includes(searchQuery.toLowerCase())
      )
    }
    ...
    if (filters.projectType !== 'all') {
      filtered = filtered.filter(nft => nft.carbonCredits.projectType === filters.projectType)
    }

    // Apply verification standard filter
    if (filters.verificationStandard !== 'all') {
      filtered = filtered.filter(nft => nft.carbonCredits.verificationStandard === filters.verificationStandard)
    }
    ...
    filtered = filtered.filter(nft => 
      nft.price >= filters.priceRange.min && nft.price <= filters.priceRange.max
    )
    ...
    switch (filters.sortBy) {
      ...
      case 'rarity':
        ...
        break
      case 'credits-high':
        filtered.sort((a, b) => b.carbonCredits.remainingCredits - a.carbonCredits.remainingCredits)
        break
      case 'credits-low':
        filtered.sort((a, b) => a.carbonCredits.remainingCredits - b.carbonCredits.remainingCredits)
        break
      case 'recent':
      default:
  ...
  const filteredCollections = useMemo(() => {
    ...
    // Apply project type filter
    if (filters.projectType !== 'all') {
      filtered = filtered.filter(collection => collection.projectType === filters.projectType)
    }

    // Apply verification standard filter
    if (filters.verificationStandard !== 'all') {
      filtered = filtered.filter(collection => collection.verificationStandard === filters.verificationStandard)
    }

    // Apply price range filter
    filtered = filtered.filter(collection => 
      collection.floorPrice >= filters.priceRange.min && collection.floorPrice <= filters.priceRange.max
    )
  ...
  const getProjectTypes = () => {
    const nftTypes = Array.from(new Set(mockNFTs.map(nft => nft.carbonCredits.projectType)))
    const collectionTypes = Array.from(new Set(mockCollections.map(collection => collection.projectType)))
    return Array.from(new Set([...nftTypes, ...collectionTypes]))
  }

  const getVerificationStandards = () => {
    const nftStandards = Array.from(new Set(mockNFTs.map(nft => nft.carbonCredits.verificationStandard)))
    const collectionStandards = Array.from(new Set(mockCollections.map(collection => collection.verificationStandard)))
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 mb-6">
            <span className="text-sm font-medium text-white/80">🌿 EcoTrade Marketplace</span>
          </div>
          ...
          <p className="text-xl text-white/70 max-w-3xl mx-auto leading-relaxed">
            Discover verified carbon credit tokens and trending collections in our innovative marketplace
          </p>
          ...
          <div className="flex justify-center space-x-8 mt-8">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-white">
                {mockCollections.filter(c => calculateTrendingScore(c) > 2).length}
              </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 border border-white/20 mb-12 shadow-2xl">
          <div className="flex items-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-white">Advanced Discovery</h2>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="credits-high" className="bg-slate-800">Credits: High to Low</option>
                <option value="credits-low" className="bg-slate-800">Credits: Low to High</option>
              </select>
            </div>

            {/* Project Type */}
            <div className="space-y-2">
              <label className="block text-sm font-medium text-white/80">Project Type</label>
              <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="space-y-2">
              ...
              >
                ...
                <option value="common" className="bg-slate-800">Common</option>
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-white/80">Verification</label>
              <select
                value={filters.verificationStandard}
                onChange={(e) => setFilters({...filters, verificationStandard: e.target.value})}
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent backdrop-blur-sm transition-all duration-300"
              >
                <option value="all" className="bg-slate-800">All Standards</option>
                {getVerificationStandards().map(standard => (
                  <option key={standard} value={standard} className="bg-slate-800">{standard}</option>
                ))}
              </select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => setFilters({...filters, burnStatus: e.target.value as any})}
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent backdrop-blur-sm transition-all duration-300"
              >
                <option value="all" className="bg-slate-800">All Status</option>
                <option value="active" className="bg-slate-800">Active</option>
                <option value="burned" className="bg-slate-800">Burned</option>
            ...
            <div className="flex space-x-2">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <div className={`px-2 py-1 rounded-full text-xs ${
                    activeTab === 'collections' ? 'bg-white/20' : 'bg-white/10'
                  }`}>
          ...
          <>
            ...
            <div className="flex justify-between items-center mb-8">
              <div className="bg-white/10 backdrop-blur-lg rounded-xl px-6 py-3 border border-white/20">
                <p className="text-white/80 font-medium">
                  Showing <span className="text-white font-bold">{startIndex + 1}-{Math.min(endIndex, filteredNFTs.length)}</span> of{' '}
                  <span className="text-white font-bold">{filteredNFTs.length}</span> NFTs
                </p>
              </div>
            ...
            {currentNFTs.length > 0 ? (
              ...
              <div className="text-center py-20">
                ...
                <h3 className="text-3xl font-bold text-white mb-4">No NFTs Found</h3>
                <p className="text-white/70 mb-8 text-lg max-w-md mx-auto">
                  Try adjusting your search criteria or filters to discover amazing carbon credit NFTs
                </p>
                <button 
                  onClick={() => {
                    setSearchQuery('')
                    ...
                    <div className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl hover:shadow-purple-500/25 transition-all duration-500 overflow-hidden border border-white/20 hover:border-white/40 transform hover:-translate-y-2 h-full flex flex-col">
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        <div className="mb-6 flex-shrink-0">
                          <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-purple-300 transition-colors line-clamp-1">{collection.name}</h3>
                          <p className="text-white/70 text-sm line-clamp-2 leading-relaxed h-10">{collection.description}</p>
                        </div>

                        {/* Stats */}
                        <div className="grid grid-cols-2 gap-6 mb-6 flex-shrink-0">
                          <div className="bg-white/10 rounded-xl p-3 border border-white/20">
                            <div className="text-xl font-bold text-emerald-400">{collection.floorPrice} ETH</div>
                            <div className="text-xs text-white/60">Floor Price</div>
                          </div>
                          <div className="bg-white/10 rounded-xl p-3 border border-white/20">
                            <div className="text-xl font-bold text-green-400">${collection.volumeTraded}M</div>
                            <div className="text-xs text-white/60">Volume</div>
                          </div>
                          <div className="bg-white/10 rounded-xl p-3 border border-white/20">
                            <div className="text-xl font-bold text-purple-400">{collectionNFTs.length}</div>
                            <div className="text-xs text-white/60">NFTs</div>
                          </div>
                          <div className="bg-white/10 rounded-xl p-3 border border-white/20">
                            <div className="text-xl font-bold text-blue-400">{collection.owners}</div>
                        ...
                        <div className="mb-6 flex-grow flex flex-col justify-end">
                          {collectionNFTs.length > 0 && (
                            <div className="mb-4">
                              <div className="text-sm font-medium text-white/80 mb-2">Featured NFTs:</div>
                              <div className="flex space-x-2">
                                {collectionNFTs.slice(0, 3).map((nft) => (
                                  <div key={nft.id} className="w-12 h-12 rounded-lg overflow-hidden border-2 border-white/20">
                                    <img
                                      src={nft.image}
                                      alt={nft.name}
                          ...
                          <div className="flex items-center justify-between text-sm">
                            <div className="flex items-center space-x-2">
                              <span className="text-white/60">{collection.projectType}</span>
                              <span className="text-white/40">•</span>
                              <span className="text-white/60">{collection.verificationStandard}</span>
                            </div>
                            <div className="text-purple-300 font-medium group-hover:text-purple-200 transition-colors">View Collection →</div>
                          </div>
                        </div>
                      </div>
                    </div>

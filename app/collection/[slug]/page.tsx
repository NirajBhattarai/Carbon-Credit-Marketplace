...
export default function CollectionPage({ params }: CollectionPageProps): JSX.Element {
  ...
  const allNFTs = params.slug === 'ocean-conservation-credits' ? oceanConservationNFTs : []
  ...
  const filteredNFTs = useMemo(() => {
    ...
    filtered = filtered.filter(nft => 
      nft.carbonCredits.remainingCredits >= filters.creditRange.min && nft.carbonCredits.remainingCredits <= filters.creditRange.max
    )
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
            <div className="flex items-end space-x-8">
              ...
              <div className="text-white">
                ...
                <p className="text-xl opacity-90 mb-6 max-w-2xl">{collection.description}</p>
                
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <div className="text-3xl font-bold">{collection.floorPrice} ETH</div>
                    <div className="text-sm opacity-75">Floor Price</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <div className="text-3xl font-bold">{collection.volumeTraded} ETH</div>
                    <div className="text-sm opacity-75">Volume</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <div className="text-3xl font-bold">{collection.totalSupply.toLocaleString()}</div>
                    <div className="text-sm opacity-75">Items</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4">
                    <div className="text-3xl font-bold">{collection.owners.toLocaleString()}</div>
              ...
              <div className="p-8">
                <div className="space-y-6">
                  {currentNFTs.slice(0, 10).map((nft, index) => (
                    <div key={nft.id} className="flex items-center justify-between py-6 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 rounded-lg px-4 transition-colors">

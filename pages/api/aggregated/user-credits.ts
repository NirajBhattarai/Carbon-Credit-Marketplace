...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log('📊 Querying user credit data from InfluxDB...')
    ...
    const sortedUsers = Object.values(userCredits).sort((a: any, b: any) => b.totalCredits - a.totalCredits)
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        totalCreditsGenerated: sortedUsers.reduce((sum: number, user: any) => sum + user.totalCredits, 0),
        totalCO2Sequestered: sortedUsers.reduce((sum: number, user: any) => sum + user.totalCO2Sequestered, 0),
        totalCO2Emitted: sortedUsers.reduce((sum: number, user: any) => sum + user.totalCO2Emitted, 0),
        users: sortedUsers,
        topPerformers: sortedUsers.slice(0, 10), // Top 10 users
        recentActivity: sortedUsers.filter((user: any) => user.lastActivity > Date.now() - 3600000) // Active in last hour
      }
    })
...
function calculateUserCredits(rawData: any[]) {
  ...
  Object.values(userCredits).forEach((user: any) => {
    ...
    user.creditsPerHour = user.creditHistory.length > 0 
      ? user.totalCredits / (user.creditHistory.length / 24) 
      : 0
    user.averageCO2PerDevice = (user.sequesterDevices + user.emitterDevices) > 0
      ? (user.totalCO2Sequestered + user.totalCO2Emitted) / (user.sequesterDevices + user.emitterDevices)
      : 0

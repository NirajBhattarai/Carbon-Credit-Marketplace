...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    try {
      ...
      console.log(`[Applications API] Found ${userApplications.length} applications`)
  ...
  if (req.method === 'POST') {
    try {
      ...
      console.log(`[Applications API] Application created with ID: ${newApplication[0].id}`)
  ...
  if (req.method === 'PUT') {
    try {
      console.log(`[Applications API] PUT request - creating API key for application`)
      console.log(`[Applications API] Request body:`, req.body)
      ...
      console.log(`[Applications API] Creating API key for application:`, applicationId)
      ...
      console.log(`[Applications API] Application query result:`, application.length > 0 ? 'found' : 'not found')
      
      if (application.length === 0) {
        console.log(`[Applications API] Application not found - ID: ${applicationId}, User: ${userId}`)
      ...
      if (application[0].apiKey) {
        ...
        return res.status(400).json({ error: 'Application already has an API key' })
      }
      ...
      console.log(`[Applications API] API key created for application: ${applicationId}`)

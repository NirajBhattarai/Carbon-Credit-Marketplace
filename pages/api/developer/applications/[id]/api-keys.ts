...
export default async function handler(
  ...
  console.log(
    `[API Keys Handler] ${method} request to /api/developer/applications/${applicationId}/api-keys`
  );
  ...
  try {
    ...
    console.log(
      `[API Keys Handler] Authentication successful for user:`,
      userId
    );
    ...
    switch (method) {
      case 'GET':
        console.log(
          `[API Keys Handler] Handling GET request for application:`,
          applicationId
        );
      ...
      case 'POST':
        console.log(
          `[API Keys Handler] Handling POST request for application:`,
          applicationId
        );
      ...
      case 'DELETE':
        console.log(
          `[API Keys Handler] Handling DELETE request for key:`,
          keyId
        );
        if (!keyId) {
          ...
          return res
            .status(400)
            .json({ error: 'Key ID is required for deletion' });
        }
...
async function handleGetApiKeys(
  ...
  try {
    console.log(
      `[Get API Keys] Looking up application ${applicationId} for user ${userId}`
    );
    ...
    console.log(
      `[Get API Keys] Application query result:`,
      application.length > 0 ? 'found' : 'not found'
    );

    if (application.length === 0) {
      console.log(
        `[Get API Keys] Application not found - ID: ${applicationId}, User: ${userId}`
      );
      return res.status(404).json({ error: 'Application not found' });
    }

    console.log(
      `[Get API Keys] Fetching API keys for application: ${applicationId}`
    );
    const keys = await db
      .select({
        id: apiKeys.id,
...
async function handleCreateApiKey(
  ...
  try {
    ...
    console.log(`[Create API Key] Request data:`, {
      name,
      expiresInDays,
      permissions,
    });
    ...
    console.log(
      `[Create API Key] Looking up application ${applicationId} for user ${userId}`
    );
    ...
    console.log(
      `[Create API Key] Application query result:`,
      application.length > 0 ? 'found' : 'not found'
    );

    if (application.length === 0) {
      console.log(
        `[Create API Key] Application not found - ID: ${applicationId}, User: ${userId}`
      );
      return res.status(404).json({ error: 'Application not found' });
    }
    ...
    console.log(
      `[Create API Key] Successfully created API key: ${newApiKey[0].id}`
    );
...
async function handleDeleteApiKey(
  ...
  try {
    console.log(
      `[Delete API Key] Looking up application ${applicationId} for user ${userId}`
    );
    ...
    console.log(
      `[Delete API Key] Application query result:`,
      application.length > 0 ? 'found' : 'not found'
    );

    if (application.length === 0) {
      console.log(
        `[Delete API Key] Application not found - ID: ${applicationId}, User: ${userId}`
      );
      return res.status(404).json({ error: 'Application not found' });
    }

    console.log(
      `[Delete API Key] Revoking API key ${keyId} for application ${applicationId}`
    );
    ...
    const result = await db
      ...
      .where(
        and(eq(apiKeys.id, keyId), eq(apiKeys.applicationId, applicationId))
      );

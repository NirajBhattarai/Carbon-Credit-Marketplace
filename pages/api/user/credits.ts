...
import { db, userCarbonCredits, userCreditHistory, users } from '../../../lib/db'
import { eq, desc, sum, avg } from 'drizzle-orm'
import { RedisService, CarbonCreditData, DashboardData } from '../../../lib/redis'
 ...
 */
async function handleGetCredits(req: AuthenticatedRequest, res: NextApiResponse) {
  try {
    // Authenticate user
 ...
 */
async function handleAddCredits(req: AuthenticatedRequest, res: NextApiResponse) {
  try {
    // Authenticate user
    const authResult = await authenticateJWT(req)
    if (!authResult) {
    ...
    if (!creditsEarned || !co2Reduced || !energySaved) {
      return res.status(400).json({
        error: 'Missing required fields: creditsEarned, co2Reduced, energySaved',
      })
    }
    ...
    const newCredits = parseFloat(currentData.credits) + parseFloat(creditsEarned)
    const newCo2Reduced = parseFloat(currentData.co2Reduced) + parseFloat(co2Reduced)
    const newEnergySaved = parseFloat(currentData.energySaved) + parseFloat(energySaved)
    const newTemperatureImpact = parseFloat(currentData.temperatureImpact) + parseFloat(temperatureImpact || '0')
    const newHumidityImpact = parseFloat(currentData.humidityImpact) + parseFloat(humidityImpact || '0')
 ...
 */
async function handleUpdateCredits(req: AuthenticatedRequest, res: NextApiResponse) {
  try {
    // Authenticate user
    const authResult = await authenticateJWT(req)
    if (!authResult) {
    ...
    // Validate required fields
    if (credits === undefined || co2Reduced === undefined || energySaved === undefined) {
      return res.status(400).json({

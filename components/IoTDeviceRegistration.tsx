...
function Toast({ message, type, onClose }: ToastProps) {
  ...
  return (
    <div
      className={`fixed top-4 right-4 z-50 max-w-sm w-full bg-white border rounded-xl shadow-xl p-4 flex items-start space-x-3 ${variants[type]} animate-in slide-in-from-right-full duration-300`}
    >
...
export function IoTDeviceRegistration({
  onDeviceRegistered,
}: IoTDeviceRegistrationProps) {
  ...
  const handleInputChange = (
    field: keyof DeviceRegistrationData,
    value: string
  ) => {
          ...
          <div className='flex items-center space-x-3'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-500'>
                Add a new device to monitor carbon credits
              </p>
            </div>
          </div>
        </ModalHeader>
            ...
            <div className='space-y-2'>
              ...
              <div className='grid grid-cols-2 gap-3'>
                ...
                >
                  <input
                    ...
                    onChange={e =>
                      handleInputChange(
                        'deviceType',
                        e.target.value as 'SEQUESTER' | 'EMITTER'
                      )
                    }
                    className='sr-only'
                  />
                  <div className='flex items-center space-x-2 mb-2'>
                    <div
                      className={`p-1 rounded ${formData.deviceType === 'SEQUESTER' ? 'bg-green-100' : 'bg-gray-100'}`}
                    >
                      <svg
                        className={`w-4 h-4 ${formData.deviceType === 'SEQUESTER' ? 'text-green-600' : 'text-gray-500'}`}
                        fill='none'
                        stroke='currentColor'
                        viewBox='0 0 24 24'
                      >
                    ...
                    <span
                      className={`font-medium ${formData.deviceType === 'SEQUESTER' ? 'text-green-700' : 'text-gray-700'}`}
                    >
                      SEQUESTER
                    </span>
                  </div>
                  <p
                    className={`text-xs ${formData.deviceType === 'SEQUESTER' ? 'text-green-600' : 'text-gray-500'}`}
                  >
                ...
                >
                  ...
                  <div className='flex items-center space-x-2 mb-2'>
                    <div
                      className={`p-1 rounded ${formData.deviceType === 'EMITTER' ? 'bg-orange-100' : 'bg-gray-100'}`}
                    >
                      <svg
                        className={`w-4 h-4 ${formData.deviceType === 'EMITTER' ? 'text-orange-600' : 'text-gray-500'}`}
                        fill='none'
                        stroke='currentColor'
                        viewBox='0 0 24 24'
                      >
                    ...
                    <span
                      className={`font-medium ${formData.deviceType === 'EMITTER' ? 'text-orange-700' : 'text-gray-700'}`}
                    >
                      EMITTER
                    </span>
                  </div>
                  <p
                    className={`text-xs ${formData.deviceType === 'EMITTER' ? 'text-orange-600' : 'text-gray-500'}`}
                  >

...
export function MQTTStatusIndicator({ className, showDetails = false }: MQTTStatusIndicatorProps) {
    ...
    <div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-3">
        <div className="text-center p-3 bg-green-50 rounded-lg">
          <div className="text-2xl font-bold text-green-600">{deviceCount.sequesters}</div>
          <div className="text-sm text-green-700">Sequester Devices</div>
        </div>
        <div className="text-center p-3 bg-orange-50 rounded-lg">
          <div className="text-2xl font-bold text-orange-600">{deviceCount.emitters}</div>
      ...
      {connectionState.reconnectAttempts > 0 && (
        <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-yellow-800 font-medium">Reconnecting...</span>
          </div>
          <p className="text-sm text-yellow-700 mt-1">
            Attempt {connectionState.reconnectAttempts} of automatic reconnection
          </p>
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-1">
        ...
        <span>MQTT</span>
        {totalDevices > 0 && (
          <span className="ml-1">({totalDevices})</span>
        )}
      </div>
    </Badge>
  )

/**
 * Carbon Credit Creator IoT Device - Production Version
 * Compatible with ESP32 and real sensors
 * 
 * This device monitors CO2 reduction activities and automatically
 * triggers carbon credit minting when thresholds are reached.
 */
...
const char* deviceId = "CARBON_CREATOR_001";
const char* deviceType = "CREATOR";
const char* location = "Solar Farm Alpha";
const char* projectName = "Renewable Energy Project";

// Sensor Pins
const int CO2_SENSOR_PIN = A0;        // MQ135 CO2 sensor
const int ENERGY_SENSOR_PIN = A1;     // Energy generation sensor
const int TEMP_HUMIDITY_PIN = 2;      // DHT22 sensor
const int ACTIVITY_LED_PIN = 4;       // Green LED for activity
const int CREDIT_LED_PIN = 5;         // Blue LED for credit generation
...
const float CO2_THRESHOLD = 1000.0;      // CO2 reduction threshold
const float ENERGY_THRESHOLD = 500.0;    // Energy generation threshold
...
void setup() {
  ...
  Serial.println("=== Carbon Credit Creator Device Starting ===");
...
void registerDevice() {
  ...
  doc["description"] = "ESP32 Carbon Credit Creator Device";
...
void readSensorData() {
  ...
  // Read energy generation sensor (simulated)
...
void checkThresholds() {
  if (!thresholdReached && 
    ...
    Serial.println("CO2 Reduction: " + String(accumulatedCO2));
    Serial.println("Energy Generated: " + String(accumulatedEnergy));
    Serial.println("========================");
    
    // Trigger credit minting
    triggerCreditMinting();
...
void triggerCreditMinting() {
  if (!deviceStatus.isConnected || !deviceRegistered) {
    Serial.println("Cannot trigger minting - device not properly connected");
    return;
  }
  ...
  if (httpResponseCode > 0) {
    String response = http.getString();
    Serial.println("Minting trigger response: " + response);
    
    if (httpResponseCode == 201) {
      Serial.println("Credit minting triggered successfully!");
      blinkCreditLED(5, 200);
    } else {
      deviceStatus.lastError = "Minting trigger failed: " + String(httpResponseCode);
    }
  } else {
    deviceStatus.lastError = "Minting trigger error: " + String(httpResponseCode);
    Serial.println("Minting trigger error: " + String(httpResponseCode));
  }
...
void playAlertTone() {
  // Play success tone for threshold reached

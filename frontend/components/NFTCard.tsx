...
export default function NFTCard({ nft, onClick }: NFTCardProps): JSX.Element {
  ...
  const getCreditPercentage = (): number => {
    return (nft.carbonCredits.remainingCredits / nft.carbonCredits.totalCredits) * 100
  }
    ...
    >
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <div className={`px-3 py-1.5 rounded-full text-xs font-bold border backdrop-blur-sm ${getRarityColor(nft.rarity)} flex items-center space-x-1`}>
        ...
        <div className="absolute top-4 right-4">
          <div className={`px-3 py-1.5 rounded-full text-xs font-bold border backdrop-blur-sm ${getBurnStatusColor(nft.burnStatus)} flex items-center space-x-1`}>
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <div className="bg-white/90 backdrop-blur-sm rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-gray-900">Carbon Credits</span>
              <span className="text-sm font-bold text-gray-900">{nft.carbonCredits.remainingCredits}/{nft.carbonCredits.totalCredits}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className={`h-2 rounded-full transition-all duration-500 ${
                  getCreditPercentage() > 50 ? 'bg-green-500' : 
                  getCreditPercentage() > 20 ? 'bg-yellow-500' : 'bg-red-500'
                }`}
                style={{ width: `${getCreditPercentage()}%` }}
              ></div>
            </div>
            <div className="text-xs text-gray-600 mt-1">
              {nft.carbonCredits.verificationStandard} • {nft.carbonCredits.projectType}
            </div>
          </div>
        </div>
      ...
      <div className="p-5">
        ...
        <div className="mb-4 space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-gray-900">{nft.carbonCredits.projectType}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600">Location</span>
            <span className="text-sm font-medium text-gray-900">{nft.carbonCredits.location}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-600">Vintage</span>
            <span className="text-sm font-medium text-gray-900">{nft.carbonCredits.vintage}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-600 font-medium">{nft.owner.name}</span>
          </div>

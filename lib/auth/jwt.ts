 ...
 */
export function generateApiKeyJWT(payload: Omit<ApiKeyPayload, 'iat' | 'exp'>): string {
  return jwt.sign(payload, JWT_SECRET, {
    expiresIn: '30d',
    issuer: 'carbon-credit-marketplace',
 ...
 */
export function generateWalletSignature(message: string, privateKey: string): string {
...
export function verifyWalletSignature(message: string, signature: string, walletAddress: string): boolean {
  ...
  const expectedSignature = crypto.createHmac('sha256', walletAddress).update(message).digest('hex')

...
import { A2AMessage, MessageType, CreditRequestMessage, CreditOfferMessage } from './a2a-protocol';
...
export class CarbonOffsetAgent extends BaseAgent {
  ...
  protected setupMessageHandlers(): void {
    this.messageHandlers.set(MessageType.CREDIT_OFFER, this.handleCreditOffer.bind(this));
    ...
    this.messageHandlers.set(MessageType.TRANSACTION_ACCEPT, this.handleTransactionAccept.bind(this));
    this.messageHandlers.set(MessageType.TRANSACTION_REJECT, this.handleTransactionReject.bind(this));
  }
  ...
  private async checkCreditNeeds(): Promise<void> {
    const totalEmissions = Array.from(this.emissionTracking.values()).reduce((sum, emissions) => sum + emissions, 0);
    ...
    if (currentCredits < targetCredits * 0.8) { // Buy when below 80% of target
      console.log(`Need to purchase credits. Current: ${currentCredits}, Target: ${targetCredits}`);
  ...
  private async requestCreditPurchase(creditsNeeded: number): Promise<void> {
    const request: CreditRequestMessage = {
      ...
      creditType: this.config.creditPreferences.preferredTypes[0] || 'SEQUESTER',
      urgency: this.config.offsetTargets.urgencyLevel,
      deadline: Date.now() + 300000, // 5 minutes
    };
    
    console.log(`Requesting ${creditsNeeded} credits at max ${this.config.budget.maxPricePerCredit} HBAR each`);
  ...
  private async handleCreditOffer(message: A2AMessage): Promise<void> {
    ...
    const pendingRequest = Array.from(this.pendingRequests.values()).find(
      req => req.creditAmount <= offer.creditAmount && req.maxPricePerCredit >= offer.pricePerCredit
    );
  ...
  private isOfferAcceptable(offer: CreditOfferMessage): boolean {
    ...
    // Check credit type
    if (!this.config.creditPreferences.preferredTypes.includes(offer.creditType)) {
      return false;
    }
    
    // Check quality
    if (this.config.creditPreferences.qualityRequirements === 'HIGH' && offer.metadata.quality !== 'HIGH') {
      return false;
    }
  ...
  private async handleTransactionAccept(message: A2AMessage): Promise<void> {
    ...
    console.log(`Transaction ${accept.transactionId} accepted by ${message.from}`);
  ...
  private async handleTransactionReject(message: A2AMessage): Promise<void> {
    ...
    console.log(`Transaction ${reject.transactionId} rejected by ${message.from}: ${reject.reason}`);
  ...
  getStatistics(): any {
    return {
      ...
      currentEmissions: Array.from(this.emissionTracking.entries()).map(([source, emissions]) => ({
        source,
        emissions,
      })),
      ...
      budget: {
        ...
        maxPricePerCredit: this.config.budget.maxPricePerCredit,
        remainingBudget: this.config.budget.monthlyBudget - this.state.performance.totalVolume,
      },
    };
  }
  ...
  simulateMonthlyBudgetRefresh(): void {
    ...
    console.log(`Monthly budget refreshed. New balance: ${this.state.hbarBalance} HBAR`);
  }
}
